...
import { CreateResourceInfoInput, CreateResourceInput, DeleteResourceInfoInput, DeleteResourcesInput, GetResourceInfosInput, GetResourcesInput, Resource, ResourceInfo, ResourceInfoList, ResourceList, UpdateResourceInfoInput } from "../models/domain-layer/legacy/resource";
...
class LegacyResourceDomain {
  ...
  public async getResources(input:GetResourcesInput): Promise<ResourceList> {
    ...
    return { resources: rows && rows?.length > 0 ? rows!.map(r => Resource.fromPartial(r as Resource)) : [] }
  }
  ...
  public async getResourceInfos(input:GetResourceInfosInput): Promise<ResourceInfoList> {
    ...
    return { resourceInfos: rows && rows?.length > 0 ? rows!.map(r => ResourceInfo.fromPartial(r as ResourceInfo)) : [] }
  }
  ...
  public async deleteResources (input:DeleteResourcesInput) {
    await queryRunner.run(
      new QueryBuilder(ResourceSchema)
        ...
        .where(ResourceSchema.columns.resourceRoleId, Operator.OPERATOR_EQUAL, {
          value: {
            $case: "intValue",
            intValue: input.resourceRoleId,
          },
